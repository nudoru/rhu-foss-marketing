...
define('nudoru.components.FloatImageView', function (require, module, exports) {
  ...
  function initialize() {
    ...
    _viewPortCoverClickStream = Rx.Observable.fromEvent(
      _viewPortCoverEl,
      _browserInfo.mouseClickEvtStr()
    ).subscribe(function () {
      hideFloatImageCover();
    });
  }
  ...
  function showImage(imageEl) {
    ...
    imgTargetX =
      vpWidth / 2 - imgTargetWidth / 2 - imgPosition.left + vpScrollLeft;
    imgTargetY =
      vpHeight / 2 - imgTargetHeight / 2 - imgPosition.top + vpScrollTop;

    var zoomImage = _DOMUtils.HTMLStrToNode(
      '<div class="' + _zoomedImageClass + '"></div>'
    );
    ...
    if (_fancyEffects) {
      ...
      var startingRot = _numberUtils.clamp(
          (imgPosition.left - vpWidth / 2) / 4,
          -75,
          75
        ),
        origin;
